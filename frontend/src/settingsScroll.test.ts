...
describe('settings scroll layout', () => {
  test('desktop settings content is the scroll container inside the fixed-height main panel', () => {
    const styles = css();
    expect(styles).toContain('.main-panel{min-width:0;height:100vh;background:var(--bg);display:grid;grid-template-columns:minmax(0,1fr);grid-template-rows:auto minmax(0,1fr) auto;overflow:hidden}');
