...
describe('mobile WebUI layout and touch affordances', () => {
  ...
  test('mobile composer expanded mode has no inner frame while keeping the previous compact control row', () => {
    ...
    expect(styles).toContain('.composer-wrap:not(.composer-compact) .composer-footer{display:flex;gap:6px;width:100%;padding:8px 10px;border-top:1px solid var(--border);background:var(--surface);overflow:visible;flex-wrap:nowrap}');
