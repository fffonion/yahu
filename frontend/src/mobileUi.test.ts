...
describe('mobile WebUI layout and touch affordances', () => {
  ...
  test('mobile composer expanded mode has no inner frame and four full-width controls', () => {
    const styles = css();
    ...
    expect(styles).toContain('.composer-wrap:not(.composer-compact) .composer-footer{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:0;width:100%;padding:0;border-top:1px solid var(--border);background:var(--surface);overflow:visible}');
    expect(styles).toContain('.composer-wrap:not(.composer-compact) .composer-footer .attach-btn,.composer-wrap:not(.composer-compact) .composer-footer .send-btn,.composer-wrap:not(.composer-compact) .composer-footer .dropdown-control{width:100%;min-width:0;height:44px}');
    expect(styles).toContain('.composer-wrap:not(.composer-compact) .composer-footer .send-btn{margin-left:0}');
  });
