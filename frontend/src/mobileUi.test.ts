...
describe('mobile WebUI layout and touch affordances', () => {
  ...
  test('mobile composer expanded mode has no inner frame while keeping the previous compact control row', () => {
    ...
    expect(styles).toContain('.composer-wrap:not(.composer-compact) .composer-box textarea{display:block;width:100%;height:64px;padding:12px 14px 48px 14px;border:0;border-radius:0;box-shadow:none;background:var(--surface);resize:none}');
    expect(styles).toContain('.composer-wrap:not(.composer-compact) .composer-box textarea:focus{border:0;box-shadow:none}');
    expect(styles).toContain('.composer-wrap:not(.composer-compact) .composer-footer{display:flex;gap:6px;width:100%;padding:8px 10px;border-top:0;background:transparent;overflow:visible;flex-wrap:nowrap;bottom:auto;top:50%;transform:translateY(-50%)}');
  ...
  test('composer collapses while reading history on desktop and mobile, then restores controls on input focus', () => {
    ...
    expect(styles).toContain('.composer-wrap.composer-compact .composer-footer{right:0;bottom:10px;width:auto;border-top:0;background:transparent;padding:0}');
  ...
  test('compact composer send button keeps the same right inset as expanded composer', () => {
    ...
    expect(styles).toContain('.composer-wrap.composer-compact .composer-footer{right:0;bottom:10px;');
    expect(styles).toContain('.composer-footer .send-btn{margin-left:auto}');
  });
