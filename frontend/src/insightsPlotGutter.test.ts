...
describe('insights chart plot gutter', () => {
  test('uses a tighter y-axis label gutter while keeping plot region and share bar aligned', () => {
    ...
    expect(css).not.toContain('.chart-y-axis{position:absolute;left:0;top:0;bottom:48px;width:34px;');
    expect(css).not.toContain('.chart-y-axis span{position:absolute;right:4px;');
  ...
  test('mobile y-axis shifts right without moving the chart plot', () => {
    ...
    expect(css).not.toContain('.chart-y-axis{left:2px;bottom:46px;width:28px;font-size:10px}.chart-y-axis span{right:2px}');
