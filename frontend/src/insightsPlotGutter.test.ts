...
describe('insights chart plot gutter', () => {
  test('uses a tighter plot gutter while keeping plot region and share bar aligned', () => {
    const app = appSource();
    const css = cssSource();
    ...
    expect(css).toContain('.chart-y-axis span{position:absolute;right:4px;');
  ...
  test('mobile y-axis stays inside the viewport instead of using a negative left offset', () => {
    const css = cssSource();
    expect(css).toContain('.chart-y-axis{left:2px;bottom:46px;width:28px;font-size:10px}.chart-y-axis span{right:2px}');
    expect(css).not.toContain('.chart-y-axis{left:-8px;bottom:46px;width:32px;font-size:10px}');
  });
});
