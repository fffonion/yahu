...
describe('collapsed subagent running count', () => {
  test('shows the number of running subagents when more than one is active', () => {
    ...
    expect(source).toContain("const autoExpandedSessionRef = useRef('');");
    expect(source).toContain('shouldAutoExpandSubagentPanel(sessionId, autoExpandedSessionRef.current, subagentCount)');
    expect(source).toContain('autoExpandedSessionRef.current = sessionId;\n    setExpanded(true);');
