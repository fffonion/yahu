...
describe('collapsed subagent running count', () => {
  test('shows the number of running subagents when more than one is active', () => {
    const source = card();
    expect(source).toContain("const runningCount = visibleSnapshot?.subagents.filter((item) => item.status === 'running').length || 0;");
    expect(source).toContain('const preview = previewSubagent(visibleSnapshot.subagents);');
